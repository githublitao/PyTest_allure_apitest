...
def api_send_check(case_data, project_dict, relevance, rel, _path, result, database_env, run_env):
    """
    接口请求并校验结果
    ...
    code, data = requestSend.send_request(case_data, project_dict["testinfo"].get("host"),
                                          project_dict["testinfo"].get("address"), relevance, _path, result, run_env)
    # 校验测试结果
    with allure.step("校验测试结果"):
        pass
    if isinstance(case_data["check"], list):
        for i in case_data["check"]:
            CheckResult.check(case_data["test_name"], i, code, data, relevance, _path, result, database_env)
    else:
        CheckResult.check(case_data["test_name"], case_data["check"], code, data, relevance, _path, result, database_env)
    # 记录关联值
    init.get_relevance(data, case_data["relevance"], rel, result)
