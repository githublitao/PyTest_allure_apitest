...
from Exception.CustomException import CaseNotFound, CaseRelevanceFileFormatError, CaseRelevanceIndexNotFound
    ...
    # 用例中参数非json格式
        ...
        try:
            with open(_path+"/"+code_json, "r", encoding="utf-8") as f:
                ...
                if not code_json:
                    result["result"] = False
                    raise CaseRelevanceIndexNotFound(code_json, test_name)
                else:
                    code_json = manage(code_json, relevance)
        # 文件不存在
        except FileNotFoundError:
            result["result"] = False
            raise CaseNotFound(code_json)
        # 文件存在，但里面存储的数据有误，json.load执行异常
        except JSONDecodeError:
            result["result"] = False
            raise CaseRelevanceFileFormatError(code_json)
    # 返回获取的期望结果
    return code_json
