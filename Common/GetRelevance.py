...
from Exception.CustomException import RelevanceFormatError


def get_relevance(data, relevance_dict, relevance, result):
    """
    从返回结果中获取关联键的值
    :param data:  返回结果
    :param relevance_dict:  关联键列表
    :param relevance: 关联对象
    :param result:  全局测试结果
    :return:
    """
    if relevance_dict:
        if not isinstance(relevance_dict, dict):
            result["result"] = False
            raise RelevanceFormatError(relevance_dict)
        for k, v in relevance_dict.items():
            value = jsonpath.jsonpath(data, expr=v)
            relevance[k] = value
    # # 关联键是否时list
...
if __name__ == "__main__":
    ...
    _relevance_list = {'name': '10', 'type': 'Web', 'version': 'KHQdpM8gAL', 'description': 'avp7tZOyiY'}
    _relevance = {'key': '$.data.key'}
    _result = {"result": True}
    print(get_relevance(_data, _relevance, _relevance_list, _result))
