...
from Common import confighttp, HostManage, ParamManage, ExpectedManage
from main import failureException


def send_request(data, host, address, relevance, _path, success, run_env):
    """
    再次封装请求
    :param data: 测试用例
    :param host: 测试地址
    ...
    """
    logging.info("="*100)
    header = ExpectedManage.read_json(data["test_name"], data["headers"], relevance, _path, success)  # 处理请求头
    logging.debug("请求头处理结果：  %s" % header)
    parameter = ExpectedManage.read_json(data["test_name"], data["parameter"], relevance, _path, success)  # 处理请求参数
    logging.debug("请求参数处理结果：  %s" % header)
    if data.get("host"):
        host = data["host"]
    if data.get("address"):
        address = data["address"]
    host = HostManage.host_manage(host, run_env)  # host处理，读取配置文件中的host
    ...
    if data["request_type"].lower() == 'post':
        if data["file"]:
            ...
            result = confighttp.post(header=header, address=data["http_type"].lower() + "://" + host + address,
        ...
        result = confighttp.get(header=header, address=data["http_type"].lower() + "://" + host + address,
        ...
        if data["file"]:
            ...
            result = confighttp.put(header=header, address=data["http_type"].lower() + "://" + host + address,
                                    request_parameter_type=data["parameter_type"], data=parameter,
                                    timeout=data["timeout"])
        ...
        result = confighttp.delete(header=header, address=data["http_type"].lower() + "://" + host + address,
                                   data=parameter, timeout=data["timeout"])
    else:
